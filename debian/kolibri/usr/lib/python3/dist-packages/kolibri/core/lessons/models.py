...
# Model Added by Vimal Rugahni
# To handle requirement of Class Channel Mapping 
class ClassRoomChannel(models.Model):

    channel_id = models.CharField(max_length=32)
    collection = models.ForeignKey(Collection, related_name='classchannels')
...
# Model Added by Vimal Rugahni
# To handle requirement of Class Channel Mapping 
class ClassChannel(AbstractFacilityDataModel):
    ...
    permissions = RoleBasedPermissions(
     ...
     # Morango fields
    morango_model_name = 'classroomchannel'

    channel_id = models.CharField(max_length=32,null=True,blank=True)
    collection = models.ForeignKey(Collection, related_name='class_channels',null=True,blank=True)
    assigned_by = models.ForeignKey(FacilityUser, related_name='assigned_classchannel', blank=False, null=False)

    
    def __str__(self):
        return 'Channel {} assigned to Class {}'.format(
            self.channel_id,
            self.collection.name,
        )
