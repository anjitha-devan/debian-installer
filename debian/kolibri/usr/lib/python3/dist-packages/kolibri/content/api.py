...
class ContentNodeFilter(IdFilter):
    ...
    siblings = CharFilter(method="filter_siblings")
    ...
    class Meta:
        ...
        fields = ['parent', 'search', 'prerequisite_for', 'has_prerequisite', 'related',
                  'recommendations_for', 'siblings', 'next_steps', 'popular', 'resume', 'ids', 'content_id', 'channel_id', 'kind']
    ...
    def filter_siblings(self, queryset, name, value):
        """
        Get siblings of this content item.
        """
        return queryset.get(pk=value).get_siblings(
            include_self=True).order_by("lft").exclude(kind=content_kinds.TOPIC)
...
class ContentNodeViewset(viewsets.ReadOnlyModelViewSet):
    ...
    @detail_route(methods=['get'])
    def first_leaf_node(self, request, **kwargs):
        # retrieve the "first" leaf node (content node)
        this_item = self.get_object()
        item = this_item.get_leafnodes().filter(channel_id=this_item.parent_id).order_by("lft").first()
        return Response({'id': item.id, 'kind': item.kind, 'title': item.title})
    ...
    # Added by Vimal Rughani
    @detail_route(methods=['get'])
    def prev_content(self, request, **kwargs):
        # retrieve the "previous" content node, according to depth-first tree traversal
        this_item = self.get_object()
        prev_item = models.ContentNode.objects.filter(available=True, tree_id=this_item.tree_id, rght__lt=this_item.lft).order_by("rght").last()
        if not prev_item:
            prev_item = this_item.get_root()
        return Response({'kind': prev_item.kind, 'id': prev_item.id, 'title': prev_item.title})

    # End of Vimal Rughani code
