...
def get_topic_progress_fraction(topic, user):
    ...
    summary = get_summary_logs(leaf_ids, user)
    logger.info(summary)
    return round(
...
def get_content_progress_total(node):
    channel_id = node.channel_id
    leaf_ids = node.get_descendants(include_self=False).order_by().exclude(kind="topic").filter(channel_id=channel_id).values_list("content_id", flat=True)
    return leaf_ids

def get_content_progress_watch(node,user):
    content_ids = get_content_progress_total(node)
    leaf_ids = ContentSummaryLog.objects.filter(user=user, content_id__in=content_ids,progress=1).count()
    return leaf_ids   
...
class ContentNodeProgressSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance, progress_fraction=None, annotate_progress_fraction=True):
        ...
        user = self.context["request"].user
        total = get_content_progress_total(instance).count()
        watched = get_content_progress_watch(instance,user)
        return {
            'pk': instance.pk,
